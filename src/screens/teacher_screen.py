...
from src.components.dialog_voice_attendance import voice_atttendance_dialog
...
def teacher_tab_take_attendance():
    ...
    with c2:
        ...
        if st.button('Run Face analysis',width='stretch',type='secondary',icon=':material/analytics:',disabled=not has_photos):
            with st.spinner('Deep scanning classroom photos......'):
                all_detected_ids={} #detected photos ki el library bna lete hai

                for idx, img in enumerate(st.session_state.attendance_images):
                    img_np=np.array(img.conver('RGB'))
    ...
    with c3:
        if st.button('Use Voice Attendance',type='primary',width='stretch', icon=':material/mic:'):
            voice_atttendance_dialog(selected_subject_id)
...
def teacher_tab_manage_subjects():
    ...
    with col2:
        if st.button('Create New Suubject', width='stretch'):
...
def teacher_tab_attendance_records():
    ...
    for r in records:
        ...
        data.append({
            ...
            "Subject Code": s['subjects']['subject_code'],
    ...
    summary['Attendance Stats']=(
        ...
        + summary['Total Count'].astype(str)+'Students'
    )
